...
Vec3 evaluatePatchPt(float u, float v) {
    float Bu[4], Bv[4];
    bernstein3(u, Bu);
    bernstein3(v, Bv);
    Vec3 P(0, 0, 0);
...
void indexToCtrlCoord(int idx, int& cx, int& cy) {
    if (idx < 0) idx = 0; if (idx > 15) idx = 15;
    cy = idx / 4; cx = idx % 4;
}

// keyboard and interaction
void adjustSelectedControlPoint(float dx, float dy, float dz) {
    int cx, cy;
...
void glutDisplay() {
    ...
    float az = camAzimuth * M_PI / 180.0f;
    float el = camElevation * M_PI / 180.0f;
    Vec3 camPos;
    ...
    glColor3f(1, 1, 1);
    char buf[256];
    sprintf_s(buf, "res = %d  (use +/-)   selected = %d (0-9,a-f)  move: j/l i/k u/o  reset: r  quit: q/esc", res, selectedIndex);
